...
extern VOID FAR PASCAL fmwrite (WORD wAddress, BYTE bValue);
...
WORD NEAR PASCAL MidiCalcFAndB (WORD wPitch, BYTE bBlock)
{
    ...
    return ((WORD) bBlock << 10) | (WORD) wPitch;
}
   ...
   for (i = 0; i < NUM2VOICES; i++)
      if (voice_table[ i ].bChannel == bChannel && (voice_table[ i ].flags1 & 1))
      {
         for (j = 0 ; j < OPS_PER_CHAN; j++ )
         {
             ...
             bnd = MidiCalcFAndB( bnd, (BYTE)((voice_table[ i ].reg5[ j ] >> 2) & 7)) ;
             fmwrite( (WORD)(32 * i + 8 * j + 5), (BYTE)(HIBYTE(bnd) | (voice_table[ i ].reg5[ j ] & 0xE0)) );
             fmwrite( (WORD)(32 * i + 8 * j + 4), (BYTE)(bnd & 0xFF));
         }
      }
...
VOID NEAR PASCAL MidiMessage (DWORD dwData)
{
    ...
    switch ((BYTE)dwData & 0xf0) {
        ...
        case 0xe0:
                // D1("\nBend");
                /* pitch bend */
                MidiPitchBend (bChannel, (USHORT)data1 | ((USHORT)data2 << 7));
